...
class TriangleViewModelUnitTest {
    @get:Rule
    ...
    fun `Should emit loading state when call generateShape`() {
        ...
        val viewModel = TriangleViewModel(shapeRepository, typeProvider)
    ...
    fun `Should emit triangle that get from ShapeRepository state when call generateShape`() =
        runBlockingTest {
            ...
            Mockito.`when`(shapeRepository.generateTriangle(anyInt())).thenReturn(shape)
    ...
    fun `Should emit error when call generateShape and get exception`() =
        runBlockingTest {
            ...
            val viewModel = TriangleViewModel(shapeRepository, typeProvider)
            viewModel.generateShapeActionResult.observeForever(generateShapeActionResultObserver)
            Mockito.`when`(shapeRepository.generateTriangle(anyInt())).thenThrow(RuntimeException())
    ...
    fun `Should emit error when call changeShapeBackground and get exception`() =
        runBlockingTest {
            ...
            Mockito.`when`(shapeRepository.generateBackground(anyInt())).thenThrow(RuntimeException())
            val shape = Triangle()
